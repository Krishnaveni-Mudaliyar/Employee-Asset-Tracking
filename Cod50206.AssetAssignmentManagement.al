codeunit 50206 "Asset Assignment Management"
{
    ...
    begin
        ...
        if AssetRequestLine."Assigned Quanity" >= AssetRequestLine."Approved Quanity" then
            Error(
                'Asset request line for %1 is already fully assigned (%2 of %3).',
                AssetRequestLine."Asset Category Code",
                AssetRequestLine."Assigned Quanity",
                AssetRequestLine."Approved Quanity");
        ...
        AssetRequestLine.Validate("Assigned Quanity",
        AssetRequestLine."Assigned Quanity" + 1);
        AssetRequestLine.Modify(true);

        CloseHeaderIfFullyAssigned(AssetRequestHeader);
    end;

    local procedure CloseHeaderIfFullyAssigned(
        var
        AssetRequestHeader: Record "Asset Request Header")
    ...
    begin
        AssetRequestLine.SetRange("Document No.",
        AssetRequestHeader."No.");
        ...
        repeat
            if AssetRequestLine."Assigned Quanity" < AssetRequestLine."Approved Quanity"
            then
                AllLinesFullyAssigned := false;
        until
            AssetRequestLine.Next() = 0;
